...
export function getLeafletHtml(params: {
  ...
  const busMarkers = busList.map((bus) => `L.marker([${bus.lat},${bus.lng}],{icon:busIcon}).bindPopup('<b>${bus.plateNumber}</b><br>${bus.busLabel}').addTo(map);`).join('\n');
  const stopMarkers = stopList.map((stop) => {
    const stopName = stop.stopName.replace(/'/g, "\\'"); // Escape single quotes
    return `L.marker([${stop.lat},${stop.lng}],{icon:stopIcon}).bindPopup('<b>${stopName}</b><br/><a href="#" onclick="postStopMessage(${stop.stopId})">Durağa Git</a>').addTo(map);`;
  }).join('\n');
  ...
  <body>
    ...
    <script>
      ...
      var stopIcon = L.icon({iconUrl:'https://cdn-icons-png.flaticon.com/512/10238/10238346.png',iconSize:[15,15]});
      var userIcon = L.icon({iconUrl:'https://cdn-icons-png.flaticon.com/512/149/149071.png',iconSize:[28,28]});
      var polylineCoords = ${polyline};
      var polyline = null;
      if (polylineCoords && polylineCoords.length) {
        polyline = L.polyline(polylineCoords, {color:'#FF6600',weight:4}).addTo(map);
      }
      ...
      // İlk markerları ekle
      updateBusMarkers(${JSON.stringify(params.busList)});
      ${stopMarkers}
      ${userMarker}

      // Eğer polyline varsa rota sınırlarına sığdır, yoksa verilen mapCenter kullan
      if (polyline) {
        map.fitBounds(polyline.getBounds(), {
          padding: [50, 50],  // Kenarlardan daha fazla boşluk bırak
          maxZoom: 11        // Maksimum zoom seviyesini sınırla
        });
      } else {
